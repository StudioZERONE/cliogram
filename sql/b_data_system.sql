...
VALUES 
  ('THEME_CONFIG', '테마 설정', '시스템 테마 수동 오버라이드 만료 시간 및 UI 설정'),
...
VALUES 
  ...
  ('STOCK_TYPE', 'Save', '예적금/파킹', 4, true),
  ('STOCK_TYPE', 'Old.Growth', '과거 매각주', 5, true)
ON CONFLICT (group_id, code) DO UPDATE 
SET code_name = EXCLUDED.code_name, sort_order = EXCLUDED.sort_order, is_active = EXCLUDED.is_active;
...
INSERT INTO public.common_codes (group_id, code, code_name, sort_order, is_active)
VALUES 
  ('MARKET_TYPE', 'NASDAQ', '나스닥 (NASDAQ)', 1, true),
  ('MARKET_TYPE', 'NYSE', '뉴욕증권거래소 (NYSE)', 2, true),
  ('MARKET_TYPE', 'NYSEARCA', 'NYSE ARCA', 3, true),
  ('MARKET_TYPE', 'KRX', '한국거래소 (KRX)', 4, true),
